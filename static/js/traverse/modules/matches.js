  ...
  template: `
    <div>
      <div class="d-flex justify-content-between align-items-center mb-1">
        ...
        <div class="d-flex align-items-center">
          ...
          <dropdown class="ml-tiny" button="btn-xs btn-gray" text="Bulk Actions" menu="dropdown-menu-right dropdown-menu-xs border" v-if="!_.isEmpty(matches)">
            ...
            <button class="dropdown-item" @click="modals.inspect = true">View in Inspect Tool</button>
            ...
            <tr class="border-top bg-xlight" v-for="match in matches">
              ...
              <td class="actions">
                ...
                <dropdown class="ml-tiny" button="btn-xs btn-gray" text="Investigate" menu="dropdown-menu-right dropdown-menu-xs border" @dropdown="dropdown">
                  ...
                  <a :href="ROOTURL + '/graph/?nodes=' + stringify(_.map(_.filter(matches, {element: 'node'}), 'properties.uuid')) + '&edges=' + match.properties.uuid" class="dropdown-item" target="_blank" v-if="match.element == 'edge'">View in Graph Tool</a>
                  <a :href="ROOTURL + '/graph/?nodes=' + match.properties.uuid" class="dropdown-item" target="_blank" v-else>View in Graph Tool</a>
                </dropdown>
              </td>
            </tr>
          </tbody>
      ...
      <modal :show="modals.refine" @cancel="modals.refine = false">
        ...
        <div slot="footer">
          <button class="btn btn-xs btn-secondary" @click="modals.refine = false">Cancel</button>
      ...
      <modal :show="modals.download" @cancel="modals.download = false">
        ...
        <div slot="footer">
          <button class="btn btn-xs btn-secondary" @click="modals.download = false">Cancel</button>
          <button class="btn btn-xs btn-gray" @click="get(false, true, false); modals.download = false">Download Matches</button>
        </div>
      </modal>
      <modal :show="modals.inspect" @cancel="modals.inspect = false">
        <div slot="header">
          <h5 class="modal-title">View in Inspect Tool</h5>
          <p class="text-muted mb-0">You can send the matches to the Inspect Tool using the options below:</p>
        </div>
        <div slot="body" class="pb-10 mb-2">
          <div class="form-group">
            <label class="label">Page:</label>
            <p class="text-muted mb-1">Each page will contain up to 1000 matches.</p>
            <input class="form-control form-control-xs" v-model="inspect.page" type="number" min="1" />
          </div>
        </div>
        <div slot="footer">
          <button class="btn btn-xs btn-secondary" @click="modals.inspect = false">Cancel</button>
          <button class="btn btn-xs btn-gray" @click="get(false, false, true); modals.inspect = false">View in Inspect Tool</button>
        </div>
      </modal>
    </div>
  `,
  ...
  computed: {
    intermediaries() {
      if (!_.isUndefined(this.qs.intermediaries)){
        ...
        return _.get(_.get(this.$store.state.intermediaries, this.api.payload.entity), this.api.payload.entity2)
      }
    },
    clean() {
      let self = this
