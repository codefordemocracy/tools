    ...
    hasedge: (state, getters) => {
      ...
      if (state.selected.length == 2 && getters.type == 'Nodes') {
        ...
        let connections = _.uniq(_.map(state.elements, function(ele) {
          if (ele.data.source == ids[0]) { return ele.data.target }
          else if (ele.data.target == ids[0]) { return ele.data.source }
          return false
        }))
    ...
    stats: (state, getters) => {
      ...
      let makevalues = function(elements) {
        ...
        let values = _.map(keys, function(k) {
          return { key: k, values: _.map(_.countBy(elements, k), function(count, value) {
