const vizdisplayer = {
  template: `
    <ul class="list-none text-xs leading-normal" v-if="!_.isEmpty(viz)">
      ...
      <li v-if="!_.includes(hide, 'id')"><strong>ID:</strong> <a class="text-primary" :href="'/view/visualization/?id=' + viz.id" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-viz-' + _.uniqueId(), ratio, window)">{{viz.id}}</a></li>
      ...
      <li v-if="!_.includes(hide, 'query')"><strong>Query:</strong> <a class="text-primary" :href="'/view/query/?id=' + viz.query" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-query-' + _.uniqueId(), ratio, window)">{{viz.query}}</a></li>
      ...
      <li v-if="!_.includes(hide, 'cloned_from') && !_.isNil(viz.cloned_from)"><strong>Cloned from:</strong> <a class="text-primary" :href="'/view/visualization/?id=' + viz.cloned_from" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-viz-' + _.uniqueId(), ratio, window)">{{viz.cloned_from}}</a></li>
