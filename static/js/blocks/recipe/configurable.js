const configurable = {
  ...
  <div class="inline">
    ...
    <span v-if="settings.version == 'results'">the {{settings.subtype}} list with ID <a :href="'/view/list/?id=' + listid + '&mode=popup'" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-list-detail-' + _.uniqueId(), 0.7, window)" :class="{'underline': !settings.unformatted}">{{listid}}</a></span>
  </div>
  `,
  props: {
    settings: {
