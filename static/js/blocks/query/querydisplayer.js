const querydisplayer = {
  ...
  template: `
    <ul class="list-none text-xs leading-normal" v-if="!_.isEmpty(query)">
      ...
      <li v-if="!_.includes(hide, 'id')"><strong>ID:</strong> <a class="text-primary" :href="'/view/query/?id=' + query.id" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-query-' + _.uniqueId(), ratio, window)">{{query.id}}</a></li>
      ...
      <li v-if="!_.includes(hide, 'lists')"><strong>Lists:</strong><span v-for="(id, i) in query.lists"><span v-if="i != 'a'">,</span> <a class="text-primary" :href="'/view/list/?id=' + id" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-list-' + _.uniqueId(), ratio, window)">{{id}}</a></span></li>
      ...
      <li v-if="!_.includes(hide, 'created_by')"><strong>Created by:</strong> <a class="text-primary" :href="'/user/' + query.created_by + '/'" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-user-' + _.uniqueId(), ratio, window)">{{query.created_by}}</a></li>
      <li v-if="!_.includes(hide, 'cloned_from') && !_.isNil(query.cloned_from)"><strong>Cloned from:</strong> <a class="text-primary" :href="'/view/query/?id=' + query.cloned_from" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-query-' + _.uniqueId(), ratio, window)">{{query.cloned_from}}</a></li>
