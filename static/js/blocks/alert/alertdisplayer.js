const alertdisplayer = {
  template: `
    <ul class="list-none text-xs leading-normal" v-if="!_.isEmpty(alert)">
      <li v-if="!_.includes(hide, 'id')"><strong>ID:</strong> <a class="text-primary" :href="'/view/alert/?id=' + alert.id" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-alert-' + _.uniqueId(), ratio, window)">{{alert.id}}</a></li>
      ...
      <li v-if="!_.includes(hide, 'query')"><strong>Query:</strong> <a class="text-primary" :href="'/view/query/?id=' + alert.query" v-on:click.prevent="POPUP(event.currentTarget.href, 'popup-view-query-' + _.uniqueId(), ratio, window)">{{alert.query}}</a></li>
