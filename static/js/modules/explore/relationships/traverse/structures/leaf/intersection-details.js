const intersection_details = {
  template: `
    <div>
      ...
      <matches :mentity="mentity" :has="has" :interactions="interactions" :api="matches[1]" :qs="settings.qs" @investigate="investigate">
        <template v-slot:header>Intermediaries</template>
        <template v-slot:intro>These are the details on the intermediaries that connect {{_.toUpper(settings.term2)}} and {{_.toUpper(settings.term3)}}.</template>
        <template v-slot:none>No intermediaries found</template>
      </matches>
    </div>
  `,
  ...
  computed: {
    mentity() {
      if (_.isArray(this.$store.state.explore.intermediaries[this.settings.entity][this.settings.entity2])) {
        return this.$store.state.explore.intermediaries[this.settings.entity][this.settings.entity2][this.settings.index]
      } else {
        return this.$store.state.explore.intermediaries[this.settings.entity][this.settings.entity2]
      }
    },
    has() {
      return {
        intermediaries: !_.isNil(this.$store.state.explore.intermediaries[this.settings.entity][this.mentity]) || !_.isNil(this.$store.state.explore.intermediaries[this.settings.entity2][this.mentity]),
  ...
  methods: {
    investigate(payload) {
      if (payload.type == 'intermediate') {
        if (!_.isNil(this.$store.state.explore.intermediaries[this.settings.entity][this.mentity])) {
        ...
        } else if (!_.isNil(this.$store.state.explore.intermediaries[this.settings.entity2][this.mentity])) {
