  ...
  template: `
    <div>
      ...
      <matches :mentity="mentity" :has="has[1]" :numbranches="numbranches" :interactions="interactions" :api="matches[0]" :qs="qs[1]" :selected="selected" @onselect="onselect" @investigate="investigate">
        <template v-slot:header>Matches</template>
        <template v-slot:intro>These are the {{settings.entity2}}s common to {{_.toUpper(settings.term)}} and {{_.toUpper(settings.term2)}} through independent expenditures.</template>
        <template v-slot:none>No matches found</template>
      </matches>
